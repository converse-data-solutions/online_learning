class Payment < ApplicationRecord
  ...
  def self.get_payments(params)
    ...
    Payment.includes(:user_course).search_using_dropdown(params[:course]).search_by_name_and_course(params[:search]).paginate(page: page, per_page: per_page)
  end
  ...
  def self.search_using_dropdown(query)
    if query.present?
      joins(user_course: [:user, :course]).where(courses: { id: query })
    else
      all
    end
  end
